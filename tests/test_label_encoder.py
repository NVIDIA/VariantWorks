...
def test_zygosity_encoder():
    ...
    variant = Variant(chrom="1", pos=240000, id="GL000235", ref='T', allele='A',
                      quality=60, filter=None, info='DP=35;AF=0.0185714', format='GT:GQ', samples=['0/0:50'],
                      zygosity=VariantZygosity.NO_VARIANT, type=VariantType.SNP, vcf='null.vcf', bam=bam)
    encoding = encoder(variant)
    assert(encoding == 0)

    variant = Variant(chrom="1", pos=240000, id="GL000235", ref='T', allele='A',
                      quality=60, filter=None, info='DP=35;AF=0.0185714', format='GT:GQ', samples=['0/1:50'],
                      zygosity=VariantZygosity.HETEROZYGOUS, type=VariantType.SNP, vcf='null.vcf', bam=bam)
    encoding = encoder(variant)
    assert(encoding == 2)
