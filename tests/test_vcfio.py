...
def test_vcf_loader_snps(monkeypatch):
    ...
    """
    vcf_bam_tuple = VCFReader.VcfBamPaths(vcf="/dummy/path.gz", bam="temp.bam", is_fp=False)
...
def test_vcf_load_fp(monkeypatch):
    ...
    """
    vcf_bam_tuple = VCFReader.VcfBamPaths(vcf="/dummy/path.gz", bam="temp.bam", is_fp=True)
    vcf_loader = MockPyVCFReader.get_vcf(monkeypatch, [vcf_bam_tuple])
    for v in vcf_loader:
...
def test_vcf_load_variant_from_multiple_files(monkeypatch):
    ...
    """
    first_vcf_bam_tuple = VCFReader.VcfBamPaths(vcf="/dummy/path.gz", bam="temp.bam", is_fp=False)
    second_vcf_bam_tuple = VCFReader.VcfBamPaths(vcf="/dummy/path.gz", bam="temp.bam", is_fp=False)
