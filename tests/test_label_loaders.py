...
from claragenomics.variantworks.types import VariantZygosity, VcfBamPaths
...
def test_vcf_load_fp():
    ...
    """
    labels = os.path.join(get_data_folder(), "candidates.vcf.gz")
    vcf_bam_tuple = VcfBamPaths(vcf=labels, bam="temp.bam", is_fp=True)
    vcf_loader = VCFLabelLoader([vcf_bam_tuple], allow_snps=True, allow_multiallele=False)
...
def test_vcf_fetch_variant():
    ...
    """
    labels = os.path.join(get_data_folder(), "candidates.vcf.gz")
    first_vcf_bam_tuple = VcfBamPaths(vcf=labels, bam="temp.bam", is_fp=False)
    second_vcf_bam_tuple = VcfBamPaths(vcf=labels, bam="temp.bam", is_fp=False)
